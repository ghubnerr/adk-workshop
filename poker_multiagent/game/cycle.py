...
class PlayerCycle:
    ...
    def __init__(
        ...
        last_raiser_step=None,
        max_steps_after_raiser=None,
        max_steps_after_big_blind=None,
    ...
    def next_player(self, step=1):
        ...
        if self.last_raiser:
            if (
                self.step_counter
                > self.last_raiser + self.max_remaining_steps_without_raising
            ):
                log.info(
                    "Max steps without raising has been reached. For example all calls after raiser."
                )
                return False

            if self.max_steps_after_raiser and (
                self.step_counter > self.max_steps_after_raiser + self.last_raiser
            ):
                log.debug("max steps after raiser has been reached")
                return False
        elif self.max_steps_after_raiser and (
            self.step_counter
            > self.max_steps_after_big_blind + self.steps_for_blind_betting
        ):
            log.debug("max steps after raiser has been reached")
            return False
    ...
    def mark_bb(self):
        ...
        self.max_steps_total = (
            self.step_counter + len(self.lst) * self.max_raises_per_player_round + 2
        )
