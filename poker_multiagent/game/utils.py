...
def _calc_score(hand):
    ...
    rcounts = {
        CARD_RANKS_ORIGINAL.find(r): "".join(hand).count(r) for r, _ in hand
    }.items()
    ...
    if score[0:2] == (3, 2) or score[0:2] == (
        ...
        if 12 in card_ranks:  # adjust if 5 high straight
            card_ranks += (-1,)
        sorted_card_ranks = sorted(
            card_ranks, reverse=True
        ...
        if flush:
            ...
            rcounts_flush = {
                CARD_RANKS_ORIGINAL.find(r): "".join(flush_hand).count(r)
                for r, _ in flush_hand
            }.items()
            score, card_ranks = zip(
                *sorted((cnt, rank) for rank, cnt in rcounts_flush)[::-1]
            )
            card_ranks = tuple(
                sorted(card_ranks, reverse=True)
